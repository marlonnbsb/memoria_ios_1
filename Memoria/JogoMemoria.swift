...
struct JogoMemoria<ConteudoCarta> where ConteudoCarta: Equatable { //Model
    ...
    private func resultadoPontuacao() {
        var resultadoFinal: Double = .zero
        ...
        for carta in cartas {
            resultadoFinal += carta.bonusRestante
        }

        UserDefaults.standard.setValue(resultadoFinal, forKey: ResultadoUsuario.resultado.rawValue)
    }
