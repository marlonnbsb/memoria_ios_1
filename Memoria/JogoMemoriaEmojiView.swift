...
struct JogoMemoriaEmojiView: View { // View
    ...
    var body: some View {
        VStack {
            if viewModel.apresentaResultado {
                apresentaResultado()
            } else {
    ...
    @ViewBuilder
    func apresentaResultado() -> some View {
        let resultado = UserDefaults.standard.double(forKey: ResultadoUsuario.resultado.rawValue)
        
        
        Text("Resultado: " + String(format: "%.2f", resultado))
        Spacer()
    }
...
struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        JogoMemoriaEmojiView(viewModel: JogoMemoriaEmoji(tema: Tema(cor: Color.clear, nomeFase: "", imgCartas: [])))
    }
}
