...
struct CartaModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ZStack {
            cartaPraCima(content: content).opacity(viradaParaCima ? 1 : 0)
            
            cartaPraBaixo().opacity(viradaParaCima ? 0 : 1)
        }
        .rotation3DEffect(Angle.degrees(rotacao), axis: (0, 1, 0))
    }
    
    @ViewBuilder
    func cartaPraCima(content: Content) -> some View {
        RoundedRectangle(cornerRadius: cornerRadius).fill(Color.white)
        RoundedRectangle(cornerRadius: cornerRadius).stroke(lineWidth: espessuraDaLinha)
        content
    }

    func cartaPraBaixo() -> some View {
